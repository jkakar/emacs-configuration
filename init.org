...
** Theme
   ...
   The [[https://github.com/protesilaos/ef-themes][ef-themes]] provide a matched light (~ef-elea-light~) and dark
   (~ef-elea-dark~) pair. Elea is the green-tinted member of the family: a
   pale sage paper and a warm near-black.

   These replaced the tinted Modus themes, which were here for the same reason
   and nearly worked. Modus holds ~fg-main~ at pure black and white, so tinting
   the ground is the only move it has, and it is a small one — the tinted
   grounds sit 2.64 and 4.37 L* off white and black. Elea moves the ink as well
   as the paper, so it can put its grounds 4.50 and 14.35 off the extremes —
   the dark is three times softer than Modus Vivendi Tinted — and still
   measure 15.86 and 13.58 contrast against Modus's 19.66 and 19.15. Trading
   a little contrast for a much softer ground is the whole point of the
   change.

   Since version 2.0.0 the ef-themes derive from the Modus themes and require
   them, so straight pulls modus-themes in as a dependency and the Modus
   palette API keeps working unchanged. ~ef-themes-get-color-value~ below is
   an alias for ~modus-themes-get-color-value~.

   Ghostty is configured with the same two themes, generated from this
   repository's checkout of the ef-themes source by
   ~~/.config/ghostty/themes/regenerate-ef.sh~, so the terminal and the editor
   match.

   emacs-plus on macOS exposes the system appearance through
   ~ns-system-appearance-change-functions~, an abnormal hook run at startup and
   whenever macOS toggles between light and dark mode. Hooking into it keeps
   Emacs in sync with the rest of the desktop.

   ~jkakar/after-load-theme-hook~ runs after each switch so that faces whose
   ...
   #+BEGIN_SRC emacs-lisp :tangle yes
     (defvar jkakar/after-load-theme-hook nil
       "Hook run after `jkakar/apply-theme-for-appearance' loads a theme.")

     (defun jkakar/get-theme-color (name)
       "Get color NAME from the active theme."
       (ef-themes-get-color-value name))

     (defun jkakar/apply-theme-for-appearance (appearance)
       "Load the Ef theme matching the system APPEARANCE (`light' or `dark')."
       (mapc #'disable-theme custom-enabled-themes)
       (load-theme (pcase appearance
                     ('light 'ef-elea-light)
                     (_ 'ef-elea-dark))
                   t)
       ...
       (if (boundp 'ns-system-appearance-change-functions)
           (progn
             (add-hook 'ns-system-appearance-change-functions
                       #'jkakar/apply-theme-for-appearance)
             ;; In daemon sessions the hook only fires once a GUI frame exists;
             ;; apply now if the appearance is already known.
             (when ns-system-appearance
               (jkakar/apply-theme-for-appearance ns-system-appearance)))
         ;; Builds without the macOS appearance hook get the dark theme.
         (jkakar/apply-theme-for-appearance 'dark)))
   #+END_SRC
   ...
   #+BEGIN_SRC emacs-lisp :tangle yes
     (use-package highlight-80+
       ...
       (defun jkakar/set-highlight-80+-face ()
         "Set the `highlight-80+' background from the active theme."
         (set-face-attribute 'highlight-80+ nil :foreground 'unspecified
                                                :background (jkakar/get-theme-color 'bg-dim)))
...
** Matching parentheses
   ...
   #+BEGIN_SRC emacs-lisp :tangle yes
     (defun jkakar/set-show-paren-match-face ()
       "Set the `show-paren-match' background from the active theme."
       (set-face-attribute 'show-paren-match nil :foreground 'unspecified
                                                 :background (jkakar/get-theme-color 'bg-paren-match)))
