...
** Theme
   ...
   The Modus themes provide a matched light (~modus-operandi~) and dark
   (~modus-vivendi~) pair with excellent contrast and readability. They ship
   with Emacs 28+, but the bundled copy trails upstream by a wide margin —
...
** Files and buffers
   ...
   jkakar/close-all-buffers~ is a touch easier.

   #+BEGIN_SRC emacs-lisp :tangle yes
     (defun jkakar/close-all-buffers ()
       (interactive)
