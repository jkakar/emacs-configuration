...
** Theme
   ...
   The Modus themes provide a matched light (~modus-operandi-tinted~) and dark
   (~modus-vivendi-tinted~) pair with excellent contrast and readability. They
   ship with Emacs 28+, but the bundled copy trails upstream by a wide margin —
   Emacs 30.2 carries 4.4.0 against upstream's 5.3.0 — so straight tracks
   ~protesilaos/modus-themes~ directly rather than using the built-in.

   The tinted variants are used rather than the plain pair. They keep the
   contrast — ~fg-main~ stays pure black and white — but move the background
   off ~#ffffff~ and ~#000000~ to ~#fbf7f0~ and ~#0d0e1c~, which reads as less
   stark without washing anything out. Ghostty is configured with the same two
   themes, generated from this repository's checkout of the modus-themes
   source, so the terminal and the editor match.
   ...
   #+BEGIN_SRC emacs-lisp :tangle yes
     ...
     (defun jkakar/apply-theme-for-appearance (appearance)
       "Load the Modus theme matching the system APPEARANCE (`light' or `dark')."
